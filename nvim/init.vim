source ~/.config/nvim/vim_plug/plugins.vim
" source ~/.config/nvim/plug_config/gruvbox.vim
source ~/.config/nvim/plug_config/dracula.vim
source ~/.config/nvim/plug_config/lightline.vim
source ~/.config/nvim/plug_config/coc.vim
" source ~/.config/nvim/plug_config/lsp_config.vim
source ~/.config/nvim/plug_config/rnvimr.vim
source ~/.config/nvim/plug_config/signify.vim
" source ~/.config/nvim/plug_config/fzf.vim
source ~/.config/nvim/general/settings.vim
source ~/.config/nvim/general/functions.vim
source ~/.config/nvim/general/mappings.vim

" luafile ~/.config/nvim/plug_config/compe_config.lua
luafile ~/.config/nvim/plug_config/colorizer.lua
